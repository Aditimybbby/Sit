...
class StripeAPI:
    ...
    async def extract_session_id(self, url: str) -> Optional[str]:
        """Extract cs_live or cs_test session ID from URL"""
        patterns = [
    ...
    async def extract_publishable_key(self, html: str) -> Optional[str]:
        """Extract publishable key from HTML"""
        patterns = [
    ...
    async def get_payment_intent_from_session(self, session_id: str, publishable_key: str) -> Optional[Dict]:
        """Get payment intent from session using Stripe's init endpoint"""
        session = await self.get_session()
        
        url = f"https://api.stripe.com/v1/payment_pages/{session_id}/init"
        ...
        try:
            async with session.post(url, data=data) as resp:
                if resp.status == 200:
                    ...
                    # Look for payment intent in various places
                    if 'payment_intent' in result:
                        payment_intent = result['payment_intent']
                        if isinstance(payment_intent, dict):
                            payment_intent = payment_intent.get('id')
                    elif 'payment_intent_id' in result:
                        payment_intent = result['payment_intent_id']
                    
                    # Extract amount
                    if 'line_item_group' in result:
                        lig = result['line_item_group']
                        if 'total' in lig:
                            amount = lig['total']
                        if 'currency' in lig:
                    ...
                    # Extract merchant
                    if 'account_settings' in result:
                        settings = result['account_settings']
                        if 'display_name' in settings:
                            merchant = settings['display_name']
    ...
    async def extract_checkout_info(self, url: str) -> Optional[Dict]:
        """Extract complete checkout info from URL"""
        session = await self.get_session()
        ...
        try:
            ...
            async with session.get(url, allow_redirects=True, timeout=10) as resp:
                ...
                if not publishable_key:
                    logger.error("No publishable key found")
                    return None
                
                # Get payment intent from session
                payment_info = await self.get_payment_intent_from_session(session_id, publishable_key)
    ...
    async def create_payment_method(self, card: Card, proxy: Optional[Proxy] = None) -> Optional[str]:
        ...
        try:
            async with session.post(url, data=data, proxy=proxy.url if proxy else None) as resp:
                result = await resp.json()
...
*Amount:* {amount}
*Session:* `{payment_info['session_id']}`
*Payment Intent:* `{payment_info['payment_intent_id'][:8]}...`
